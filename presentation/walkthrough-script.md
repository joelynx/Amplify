...
Two presenters: **Karthik** (talks only) and **Joel** (drives the laptop, talks the math). Start at `https://amplify-karth.vercel.app`, logged out. Have a second **incognito window** pre-opened and signed in as a TA/faculty user, parked on `/author`. Have the magic-link email already open in a second tab on the main window.
...
>
> This is Amplify. It's hosted, it's live, and it has 401 curated questions right now. I'll walk you through what a student would actually do to use it.

---

## Scene 2 — Sign in (~30s)

Click **Sign in to practice** → on `/auth/login`, type the demo email → click **Send magic link** → switch to the email tab → click the link → land back on the app, signed in.

**Karthik:**
> Sign-in is a magic link from your institute address — no passwords. The app also works anonymously: you can start a session and the system tracks your mastery on a cookie. Signing in later just keeps it.
...
Click a tag once (compulsory), again (excluded), again (off). Then select only **Proof** under types.

**Karthik:**
> Tags are tri-state too — required, excluded, or ignored. So you can say "give me Calculus questions but skip the proof-heavy ones." Try doing that with a PDF.
